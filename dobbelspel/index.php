...
// auteur Emre Bas
        ...
        switch ($_POST['action']) {
            ...
            case 'reset':
                ...
                if (isset($_COOKIE['DiceScores'])) {
                    setcookie("DiceScores", "", time() - 3600, "/");
                    $_COOKIE['DiceScores'] = "";
                }
                break;
            default:
    ...
    <form method="POST" id="diceForm">
        ...
        <input type="text" id="player" name="player" placeholder="username" required <?php if ($game->isActive()) {
                                                                                            echo ("disabled");
                                                                                        } ?> value="<?php echo ($game->getUsername()); ?>">
        <br>
        <label for="sides">Number of Sides per Die:</label>
        <input type="range" id="sides" name="sides" min="2" max="20" value="<?php echo ($game->getNumberSides()); ?>" <?php if ($game->isActive()) {
                                                                                                                            echo ("disabled");
                                                                                                                        } ?>>
        <output for="sides" id="sidesValue"><?php echo ($game->getNumberSides()); ?></output>
        <br>
        <label for="numDice">Number of Dice:</label>
        <input type="range" id="numDice" name="numDice" min="2" max="8" value="<?php echo ($game->getNumberDice()); ?>" <?php if ($game->isActive()) {
                                                                                                                            echo ("disabled");
                                                                                                                        } ?>>
        ...
        <br>
        <button type="submit" id="startGame" name="action" value="start" style="display: <?php if ($game->isActive()) {
                                                                                                echo ("none");
                                                                                            } else {
                                                                                                echo ("inline-block");
                                                                                            } ?>;">Start Game</button>
        <button type="submit" id="lowerBet" name="action" value="lower" style="display: <?php if ($game->isActive()) {
                                                                                            echo ("inline-block");
                                                                                        } else {
                                                                                            echo ("none");
                                                                                        } ?>;">Bet Lower or Equal</button>
        <button type="submit" id="higherBet" name="action" value="higher" style="display: <?php if ($game->isActive()) {
                                                                                                echo ("inline-block");
                                                                                            } else {
                                                                                                echo ("none");
                                                                                            } ?>;">Bet Higher</button>
        <button type="submit" id="forfeit" name="action" value="forfeit" style="display: <?php if ($game->isActive()) {
                                                                                                echo ("inline-block");
                                                                                            } else {
                                                                                                echo ("none");
                                                                                            } ?>;">Forfeit</button>
